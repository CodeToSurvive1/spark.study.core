package io.github.codetosurvive.spark.test;
...
import io.github.codetosurvive.spark.util.DateUtils;
import io.github.codetosurvive.spark.util.StringUtils;
...
public class MockData {
	...
	public static void mock(JavaSparkContext sc, SQLContext sqlContext) {
		...
		String[] searchKeywords = new String[] { "火锅", "蛋糕", "重庆辣子鸡", "重庆小面", "呷哺呷哺", "新辣道鱼火锅", "国贸大厦", "太古商场", "日本料理",
				"温泉" };
		...
		for (int i = 0; i < 100; i++) {
			...
			for (int j = 0; j < 10; j++) {
				...
				String sessionid = UUID.randomUUID().toString().replace("_", "");
		...
		StructType schema = DataTypes
				.createStructType(Arrays.asList(DataTypes.createStructField("date", DataTypes.StringType, true),
		...
		StructType schema2 = DataTypes
				.createStructType(Arrays.asList(DataTypes.createStructField("user_id", DataTypes.LongType, true),
		...
		DataFrame dataFrame2 = sqlContext.createDataFrame(rowsRDD, schema2);

		dataFrame2.registerTempTable("user_info");
	}
